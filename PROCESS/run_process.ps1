param(
    [string]$Station = "KUUJJUAQ+UA",  # inuvik not done
    [int]$StartYear = 2018,  # 2018
    [int]$EndYear = 2025
)
...
for ($y = $StartYear; $y -le $EndYear; $y++) {
    ...
    python -c "import sys; sys.path.insert(0, '.'); from patched_combine_dataset import combine_year; combine_year('$Station', $y)"
